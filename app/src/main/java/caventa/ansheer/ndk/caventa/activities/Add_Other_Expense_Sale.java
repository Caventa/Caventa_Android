...
import caventa.ansheer.ndk.caventa.commons.Spinner_Utils;
...
public class Add_Other_Expense_Sale extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.add_other_expense);
        ...
        pick_date.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Date_Picker_Utils.show_date_picker(Add_Other_Expense_Sale.this, date, calendar);
            }
        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.menu_item_save) {
            save_menu_item = item;
            attempt_other_expense_sale_save();
            return true;
        }
        return super.onOptionsItemSelected(item);
    }

    private void attempt_other_expense_sale_save() {
        if (other_expense_sale_save_task != null) {
            other_expense_sale_save_task.cancel(true);
            other_expense_sale_save_task = null;
        }

        Validation_Utils.reset_errors(new EditText[]{txt_description, txt_amount});

        Pair<Boolean, EditText> empty_check_result = Validation_Utils.empty_check(new Pair[]{new Pair(txt_description, "Please Enter " + (category == 0 ? "Other Expense" : "Other Sale") + " Name..."),
                new Pair(txt_amount, "Please Enter " + (category == 0 ? "Other Expense" : "Other Sale") + " Amount...")});
        ...
        if (empty_check_result.first) {
            ...
            Pair<Boolean, EditText> zero_check_result = Validation_Utils.zero_check(new Pair[]{new Pair(txt_amount, "Please Enter Valid " + (category == 0 ? "Other Expense" : "Other Sale") + " Amount...")});
    ...
    private void execute_other_expense_sale_save_task() {
        if (Network_Utils.isOnline(application_context)) {
            ...
            other_expense_sale_save_task = new Other_Expense_Sale_Save_Task(txt_description.getText().toString(),Double.parseDouble( txt_amount.getText().toString()), this);
            save_menu_item.setEnabled(false);
            other_expense_sale_save_task.execute((Void) null);
        } else {
    ...
    private Other_Expense_Sale_Save_Task other_expense_sale_save_task = null;

    /* Represents an asynchronous login task used to authenticate the user. */
    public class Other_Expense_Sale_Save_Task extends AsyncTask<Void, Void, String[]> {

        String task_other_expense_sale_description;
        Double task_other_expense_sale_amount;
        AppCompatActivity current_activity;

        Other_Expense_Sale_Save_Task(String other_expense_sale_description, Double other_expense_sale_amount, AppCompatActivity current_activity) {
            task_other_expense_sale_description = other_expense_sale_description;
            task_other_expense_sale_amount = other_expense_sale_amount;
        ...
        protected String[] doInBackground(Void... params) {
            Log.d(General_Data.TAG, task_other_expense_sale_description + "," + task_other_expense_sale_amount);

            return perform_http_client_network_task((category == 0 ? General_Data.SERVER_IP_ADDRESS + "/android/add_other_expense.php" : General_Data.SERVER_IP_ADDRESS + "/android/add_other_sale.php"), new Pair[]{new Pair("description", task_other_expense_sale_description), new Pair("amount", String.valueOf(task_other_expense_sale_amount))});
        }
            ...
            try {
                ...
                switch (response_code) {
                    case "0":
                        ...
                        Activity_Utils.start_activity_with_integer_extras_and_finish(current_activity, Other_Expense_Sale_Ledger.class, new Pair[]{new Pair("position", category)});
                        break;
                    case "1":
    ...
    public void onBackPressed() {
        ...
        Activity_Utils.start_activity_with_integer_extras_and_finish(this, Other_Expense_Sale_Ledger.class, new Pair[]{new Pair("position", 0)});
        }
    ...
    public void show_uncancelled_yes_no_confirmation_for_unsaved_data_and_finish_on_yes(final Context context) {
        ...
        delete_confirmation_dialog.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                dialog.cancel();
                Activity_Utils.start_activity_with_integer_extras_and_finish(activity_context, Other_Expense_Sale_Ledger.class, new Pair[]{new Pair("position", 0)});
            }
        });
