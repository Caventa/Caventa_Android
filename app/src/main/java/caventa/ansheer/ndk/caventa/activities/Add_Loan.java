...
public class Add_Loan extends AppCompatActivity {
    ...
    private EditText txt_installment_amount;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pick_date.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Date_Picker_Utils.show_date_picker(Add_Loan.this, date, calendar);
            }
        });
...
//        Spinner_Utils.attach_items_to_simple_spinner(application_context, spinner_category, new ArrayList<>(Arrays.asList("Other Expenses", "Other Sales")));
    ...
    private void attempt_loan_save() {
        if (loan_save_task != null) {
            loan_save_task.cancel(true);
            loan_save_task = null;
        }

        Validation_Utils.reset_errors(new EditText[]{txt_description, txt_amount});

        Pair<Boolean, EditText> empty_check_result = Validation_Utils.empty_check(new Pair[]{new Pair(txt_description, "Please Enter Loan Name..."),
                new Pair(txt_amount, "Please Enter Loan Amount..."),
                new Pair(txt_installment_amount, "Please Enter Loan Installment Amount...")});
        ...
        if (empty_check_result.first) {
            ...
            Pair<Boolean, EditText> zero_check_result = Validation_Utils.zero_check(new Pair[]{new Pair(txt_amount, "Please Enter Valid Loan Amount..."),
                    new Pair(txt_installment_amount, "Please Enter Loan Installment Amount...")});
            if (zero_check_result.first) {
                zero_check_result.second.requestFocus();
            } else {
                execute_loan_save_task();
            }
        }
    }

    private void execute_loan_save_task() {
        if (Network_Utils.isOnline(application_context)) {
            showProgress(true, application_context, mProgressView, mLoginFormView);
            loan_save_task = new Loan_Save_Task(txt_description.getText().toString(), Double.parseDouble(txt_amount.getText().toString()), Double.parseDouble(txt_installment_amount.getText().toString()), this);
            save_menu_item.setEnabled(false);
            loan_save_task.execute((Void) null);
        } else {
    ...
    private Loan_Save_Task loan_save_task = null;

    /* Represents an asynchronous login task used to authenticate the user. */
    public class Loan_Save_Task extends AsyncTask<Void, Void, String[]> {

        String task_loan_description;
        Double task_loan_amount,task_loan_installment_amount;
        AppCompatActivity current_activity;

        Loan_Save_Task(String loan_description, Double loan_amount, double loan_installment_amount, AppCompatActivity current_activity) {
            task_loan_description = loan_description;
            task_loan_amount = loan_amount;
            task_loan_installment_amount=loan_installment_amount;
        ...
        protected String[] doInBackground(Void... params) {
            Log.d(General_Data.TAG, task_loan_description + "," + task_loan_amount);

            return perform_http_client_network_task(General_Data.SERVER_IP_ADDRESS + "/android/add_loan.php", new Pair[]{new Pair("description", task_loan_description), new Pair("loan_amount", String.valueOf(task_loan_amount)), new Pair("installment_amount", String.valueOf(task_loan_installment_amount))});
        }
            ...
            try {
                ...
                switch (response_code) {
                    case "0":
                        ...
                        Activity_Utils.start_activity_with_finish(current_activity, Loan_Ledger.class);
                        break;
                    case "1":
    ...
    public void onBackPressed() {
        if (form_check()) {
            ...
            Activity_Utils.start_activity_with_finish(this, Loan_Ledger.class);
        }
    ...
    public void show_uncancelled_yes_no_confirmation_for_unsaved_data_and_finish_on_yes(final Context context) {
        ...
        delete_confirmation_dialog.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                dialog.cancel();
                Activity_Utils.start_activity_with_finish(activity_context, Loan_Ledger.class);
            }
        });
